...
function App() {
  ...
  const handleGoogleSignIn = () => {
    ...
    if (window.google?.accounts?.id) {
      const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID || "1032549298372-placeholder.apps.googleusercontent.com";
      try {
        window.google.accounts.id.initialize({
          ...
          callback: async (response) => {
            try {
              ...
              setToken(data.token);
              setUser(data.user);
              showToast("Welcome! Signed in with Google.", "default");
            } catch (err) {
        ...
        setAuthLoading(false);
        setAuthError("Google Sign-In failed to initialize. Please try again.");
      }
    } else {
      setAuthLoading(false);
      setAuthError("Google Identity Service is loading. Please check your connection and try again.");
    }
  };
  ...
  const handleGithubSignIn = async () => {
    ...
    try {
      ...
      if (data && data.url) {
        ...
        throw new Error("Invalid GitHub authorization URL received");
      }
    } catch (err) {
      setAuthLoading(false);
      setAuthError(err.message || "GitHub authentication is not configured yet. Please add GITHUB_CLIENT_ID to your environment.");
    }
  };
