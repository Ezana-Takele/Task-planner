...
function App() {
  ...
  // Social Sign-In Modal State ('google' | 'github' | null)
  ...
  // OAuth URL Callback Handler & Google Identity script loader
  useEffect(() => {
    ...
    if (tokenParam) {
      localStorage.setItem("tp_token", tokenParam);
      setToken(tokenParam);
      window.history.replaceState({}, document.title, window.location.pathname);
      showToast("Welcome! Signed in successfully.", "default");
    } else if (oauthError) {
      setAuthError(`OAuth sign-in notice: ${oauthError.replace(/_/g, " ")}`);
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  ...
  const handleGithubSignIn = () => {
    setSocialModal("github");
    setAuthError("");
    setCustomSocialEmail("");
  };

  const handleInstantSocialLogin = async (provider, email, name, avatarUrl) => {
    setAuthLoading(true);
    setAuthError("");
    try {
      const data = await apiRequest("/auth/social-instant", {
        method: "POST",
      ...
      setUser(data.user);
      setSocialModal(null);
      showToast(`Welcome! Signed in with ${provider === "github" ? "GitHub" : "Google"}.`, "default");
    } catch (err) {
      ...
      <div className={`app-canvas theme-${theme} landing-canvas`}>
                  ...
                  {authOtpPending ? (
                    ...
                    <form onSubmit={handleAuthSubmit} className="ui-form">
                      ...
                      <div className="oauth-buttons-grid">
                        ...
                        >
                          <svg className="oauth-icon" viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
        ...
        {socialModal === "google" && (
          <div className="modal-backdrop" onClick={() => setSocialModal(null)}>
            <div className="modal-card social-modal-card" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div>
                    <h3 className="modal-heading">Sign in with Google</h3>
                    <p className="modal-subheading">Choose an account to continue to TaskPlanner</p>
                  </div>
                </div>
                <button className="icon-btn-ghost" onClick={() => setSocialModal(null)}>
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="social-account-list">
                <button
                  ...
                  onClick={() => handleInstantSocialLogin("google", "drtakeleezana@gmail.com", "Ezana Takele")}
                  disabled={authLoading}
                >
                  <div className="social-avatar-circle" style={{ background: "#4285F4" }}>E</div>
                  <div className="social-account-info">
                    <span className="social-account-name">Ezana Takele</span>
                    <span className="social-account-email">drtakeleezana@gmail.com</span>
                  </div>
                ...
                <button
                  type="button"
                  className="social-account-item"
                  onClick={() => handleInstantSocialLogin("google", "ezanaeleni@gmail.com", "Elen Takele")}
                  disabled={authLoading}
                >
                  <div className="social-avatar-circle" style={{ background: "#34A853" }}>E</div>
                  <div className="social-account-info">
                    <span className="social-account-name">Elen Takele</span>
                    <span className="social-account-email">ezanaeleni@gmail.com</span>
                  </div>
                  <ChevronRight size={16} className="text-muted" />
                </button>
              </div>

              <div className="social-custom-section">
                <p className="social-custom-label">Or enter any Google account email:</p>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    if (customSocialEmail.trim()) {
                      handleInstantSocialLogin("google", customSocialEmail.trim(), customSocialEmail.split("@")[0]);
                    }
                  }}
                ...
                >
                  <input
                    ...
                    style={{ flex: 1 }}
                  />
                  <button type="submit" className="btn-primary" disabled={authLoading || !customSocialEmail.trim()}>
            ...
            <div className="modal-card social-modal-card" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div>
                    <h3 className="modal-heading">Sign in with GitHub</h3>
                    <p className="modal-subheading">Connect with your GitHub profile</p>
                  </div>
                </div>
                <button className="icon-btn-ghost" onClick={() => setSocialModal(null)}>
                  <X size={16} />
              ...
              <div className="social-github-card">
                <div className="github-card-profile">
                  <img
                    src="https://github.com/Ezana-Takele.png"
                    alt="Ezana Takele"
                  ...
                  <div className="github-profile-details">
                    <span className="github-name">Ezana Takele</span>
                    <span className="github-username">@Ezana-Takele</span>
                    <span className="github-repo-badge">Ezana-Takele/Task-planner</span>
                  </div>
                </div>

                <div className="github-permissions-list">
                  <div className="github-permission-item">
                    <Check size={14} className="text-emerald" />
                    <span>Instant access without passwords or OTP</span>
                  </div>
                  <div className="github-permission-item">
                    <Check size={14} className="text-emerald" />
                    <span>Sync developer tasks & workspace</span>
                  </div>
                </div>

                <button
                  type="button"
                  className="btn-primary btn-github-authorize"
                  onClick={() => handleInstantSocialLogin("github", "drtakeleezana@gmail.com", "Ezana Takele", "https://github.com/Ezana-Takele.png")}
                  disabled={authLoading}
                >
                  {authLoading ? "Signing In..." : "Continue as @Ezana-Takele"}
                </button>
              </div>
            </div>
          </div>
